...
const RedisDemo = () => {
  ...
  return (
    <DemoContainer>
      ...
      {/* Advanced Operations */}
      <Section>
        <SectionTitle>🚀 Advanced Operations</SectionTitle>
        <ExplanationBox>
          <h4>Zaawansowane operacje Redis</h4>
          <p>
            Redis oferuje zaawansowane funkcje jak transakcje (MULTI/EXEC), pipeline, 
            pub/sub, oraz lua scripting dla atomowych operacji.
          </p>
        </ExplanationBox>
        
        <OperationGrid>
          <OperationCard>
            <OperationTitle>Pipeline Operations</OperationTitle>
            <p>Wykonanie wielu komend w jednym batch'u dla lepszej wydajności.</p>
            <Button 
              onClick={() => performPipelineDemo()}
              disabled={loading.pipeline}
            >
              {loading.pipeline ? 'Wykonuję...' : 'Demo Pipeline'}
            </Button>
            
            {results.pipeline && (
              <ResultContainer>
                <p><strong>Operacji wykonanych:</strong> {results.pipeline.operations}</p>
                <p><strong>Czas wykonania:</strong> {results.pipeline.time}ms</p>
                <RedisCommand>
                  MULTI<br/>
                  SET key1 value1<br/>
                  SET key2 value2<br/>
          ...
          <OperationCard>
            <OperationTitle>Atomic Operations</OperationTitle>
            <p>Transakcje zapewniające atomowość operacji.</p>
            <Button 
              onClick={() => performAtomicDemo()}
              disabled={loading.atomic}
            >
              {loading.atomic ? 'Wykonuję...' : 'Demo Transakcji'}
            </Button>
            
            {results.atomic && (
              <ResultContainer>
                <p><strong>Status transakcji:</strong> {results.atomic.success ? 'Sukces' : 'Błąd'}</p>
                <p><strong>Operacje:</strong> Transfer punktów między użytkownikami</p>
                <RedisCommand>
                  WATCH user:123:points user:456:points<br/>
                  MULTI<br/>
                  DECRBY user:123:points 10<br/>
                  INCRBY user:456:points 10<br/>
                  EXEC
                </RedisCommand>
              </ResultContainer>
            )}
          ...
          <OperationCard>
            <OperationTitle>Pub/Sub Messaging</OperationTitle>
            <p>System publikowania i subskrypcji wiadomości.</p>
            <Button 
              onClick={() => performPubSubDemo()}
              disabled={loading.pubsub}
            >
              {loading.pubsub ? 'Publikuję...' : 'Demo Pub/Sub'}
            </Button>
            
            {results.pubsub && (
              <ResultContainer>
                <p><strong>Kanał:</strong> {results.pubsub.channel}</p>
                <p><strong>Subskrybentów:</strong> {results.pubsub.subscribers}</p>
                <p><strong>Wiadomość:</strong> {results.pubsub.message}</p>
                <RedisCommand>
                  PUBLISH chat:notifications "New message in #general"
                </RedisCommand>
              </ResultContainer>
            )}
          </OperationCard>
          ...
          <OperationCard>
            <OperationTitle>Lua Scripting</OperationTitle>
            <p>Wykonywanie skryptów Lua po stronie serwera.</p>
            <Button 
              onClick={() => performLuaDemo()}
              disabled={loading.lua}
            >
              {loading.lua ? 'Wykonuję...' : 'Demo Lua Script'}
            </Button>
            
            {results.lua && (
              <ResultContainer>
                <p><strong>Wynik skryptu:</strong> {results.lua.result}</p>
                <p><strong>Operacja:</strong> Atomowe zwiększenie z limitem</p>
                <RedisCommand>
                  EVAL "if redis.call('get',KEYS[1]) &lt; ARGV[1] then return redis.call('incr',KEYS[1]) else return 0 end" 1 counter 100
                </RedisCommand>
              </ResultContainer>
            )}
          </OperationCard>
      ...
      <Section>
        ...
        <p>Testuj wydajność różnych operacji Redis:</p>
        
        <OperationGrid>
          <OperationCard>
            <OperationTitle>Benchmark STRING operations</OperationTitle>
            <Button 
              onClick={() => runBenchmark('string', 1000)}
              disabled={loading.benchmark}
            >
              {loading.benchmark ? 'Testowanie...' : 'Test 1000 SET/GET'}
            </Button>
          </OperationCard>
          
          <OperationCard>
            <OperationTitle>Benchmark HASH operations</OperationTitle>
            <Button 
              onClick={() => runBenchmark('hash', 1000)}
              disabled={loading.benchmark}
            >
              {loading.benchmark ? 'Testowanie...' : 'Test 1000 HSET/HGET'}
            </Button>
          </OperationCard>
          
          <OperationCard>
            <OperationTitle>Benchmark LIST operations</OperationTitle>
            <Button 
              onClick={() => runBenchmark('list', 1000)}
              disabled={loading.benchmark}
            >
              {loading.benchmark ? 'Testowanie...' : 'Test 1000 LPUSH/LPOP'}
            </Button>
          </OperationCard>
          
          <OperationCard>
            <OperationTitle>Memory Usage</OperationTitle>
            <Button 
              onClick={() => checkMemoryUsage()}
              disabled={loading.memory}
            >
              {loading.memory ? 'Sprawdzanie...' : 'Sprawdź zużycie pamięci'}
            </Button>
          </OperationCard>
        </OperationGrid>
        ...
        {results.benchmark && (
          <ResultContainer style={{ marginTop: '20px' }}>
            <h4>Wyniki benchmarku:</h4>
            <p><strong>Operacja:</strong> {results.benchmark.operation}</p>
            <p><strong>Iteracje:</strong> {results.benchmark.iterations}</p>
            <p><strong>Całkowity czas:</strong> {results.benchmark.totalTime}ms</p>
            <p><strong>Średni czas:</strong> {results.benchmark.avgTime?.toFixed(2)}ms</p>
            <p><strong>Throughput:</strong> {results.benchmark.throughput?.toFixed(2)} ops/sec</p>
          </ResultContainer>
        )}
        
        {results.memory && (
          <ResultContainer style={{ marginTop: '20px' }}>
            <h4>Wykorzystanie pamięci:</h4>
            <p><strong>Używana pamięć:</strong> {results.memory.used}</p>
            <p><strong>Peak memory:</strong> {results.memory.peak}</p>
            <p><strong>Liczba kluczy:</strong> {results.memory.keys}</p>
            <p><strong>Średni rozmiar klucza:</strong> {results.memory.avgKeySize}</p>
          </ResultContainer>
        )}
      </Section>
      ...
      {/* Redis CLI Reference */}
      <Section>
        <SectionTitle>📋 Redis CLI Quick Reference</SectionTitle>
        <ExplanationBox>
          <h4>Przydatne komendy Redis CLI</h4>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginTop: '15px' }}>
            <div>
              <h5>🔧 Diagnostyka i monitoring:</h5>
              <RedisCommand style={{ fontSize: '0.8rem', margin: '10px 0' }}>
                INFO memory          # Informacje o pamięci<br/>
                INFO stats           # Statystyki Redis<br/>
                MONITOR              # Monitor operacji w real-time<br/>
                CLIENT LIST          # Lista połączonych klientów<br/>
                SLOWLOG GET 10       # 10 najwolniejszych operacji
              </RedisCommand>
            </div>
            <div>
              <h5>⚙️ Konfiguracja i zarządzanie:</h5>
              <RedisCommand style={{ fontSize: '0.8rem', margin: '10px 0' }}>
                CONFIG GET maxmemory # Sprawdź limit pamięci<br/>
                CONFIG SET maxmemory 1gb # Ustaw limit<br/>
                FLUSHDB              # Wyczyść bieżącą bazę<br/>
                FLUSHALL             # Wyczyść wszystkie bazy<br/>
                BGSAVE               # Zapisz snapshot w tle
              </RedisCommand>
            </div>
          </div>
        </ExplanationBox>
